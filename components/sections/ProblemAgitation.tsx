      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 flex flex-col lg:flex-row gap-10 sm:gap-12 lg:gap-24 items-center">
        ...
        >
          ...
          <motion.div variants={fadeUpVariants} custom={0} className="flex items-center gap-2.5 sm:gap-3">
            ...
            <span className="font-mono text-[10px] sm:text-xs font-semibold tracking-[0.15em] sm:tracking-[0.2em] text-brand-terracotta uppercase">
              El Agujero Negro // Tu Mayor Pérdida
            </span>
          </motion.div>
          ...
          <motion.h2 variants={fadeUpVariants} custom={1} className="font-sans text-[1.75rem] sm:text-4xl md:text-5xl lg:text-6xl font-extrabold sm:font-black tracking-tight text-white leading-[1.05]">
            <span className="block whitespace-nowrap">70% de tu pauta</span>
            <span className="block font-accent italic tracking-normal text-brand-terracotta whitespace-nowrap">
              muere en silencio.
            </span>
          </motion.h2>
          ...
          <motion.p variants={fadeUpVariants} custom={2} className="font-sans text-base sm:text-lg lg:text-xl text-brand-bone/80 leading-relaxed font-light mt-1 sm:mt-2 max-w-none sm:max-w-lg">
            En e-commerce, cada peso se mide: clicks, conversiones, ROAS. Pero en tu WhatsApp hay un agujero negro. Invertís en Meta Ads para que el lead llegue, pregunta el precio, y desaparece. <br /><strong className="text-white font-bold border-b-2 border-brand-terracotta pb-0.5">Eso es demanda oscura.</strong>
            <span className="block mt-3 sm:mt-4 text-brand-bone/50 text-sm sm:text-base">
              Capital que ya pagaste y nunca pudiste cobrar. Sin trazabilidad. Sin datos. Millones evaporándose por día.
            </span>
          </motion.p>
        </motion.div>
        ...
        >
          {[
            ...
            >
              {/* Glow interno radial sutil al fondo — sincronizado con features */}
              ...
              <div className="mt-0.5 sm:mt-1 flex-shrink-0 p-2.5 sm:p-3 md:p-4 rounded-lg border border-brand-bone/10 bg-brand-bone/5 text-brand-bone transition-all duration-300 transform-gpu lg:group-hover:scale-110 lg:group-hover:text-brand-terracotta lg:group-hover:border-brand-terracotta/20 lg:group-hover:bg-brand-terracotta/10">
                {dolor.icon}
              </div>
